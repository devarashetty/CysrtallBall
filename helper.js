 ...
 */
function calculateBodyMeasurements(pose){
  const poseKeypoints = pose.keypoints;
  poseKeypoints.forEach(function(keypt){
    var x1, x2;
    var y1, y2;
    if(keypt.part == "leftShoulder"){
      x1=keypt.position.x;
      y1=keypt.position.y;
    }
    if(keypt.part == "rightShoulder"){
      x2=keypt.position.x;
      y2=keypt.position.y;
    }
...
function detectPoseInRealTime(video, net) {
  ...
  async function poseDetectionFrame() {
    ...
    switch (guiState.algorithm) {
      case 'single-pose':
        ...
        document.getElementById("output1").innerHTML = myJSON;
        calculateBodyMeasurements(pose);
 ...
 //console.log(window.URL);
  ...
  $(video).on('pause', function() {
    ...
    console.log(average(arrEar));
    console.log(average(arrEye));
